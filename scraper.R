...
for(iter in max(1, length(results_from_companies_house)):min(pages_to_get, max_iterations)){
  ...
  results_from_companies_house[[iter]] <- data.frame(company_ids, company_names)
}
close(pb)